//! # Orbit Camera — Touch Viewer Example
//!
//! Shows touch-input configuration: one-finger orbit, two-finger pan, pinch zoom.
//! Desktop mouse controls remain active alongside touch.
...
// ---------------------------------------------------------------------------
// Pane — touch-relevant settings are highlighted
// ---------------------------------------------------------------------------

#[derive(Resource, Debug, Clone, Copy, PartialEq, Pane)]
#[pane(title = "Orbit Camera — Touch", position = "top-right")]
struct OrbitPane {
    ...
    #[pane(toggle)]
    touch_enabled: bool,
    #[pane(slider, min = 0.001, max = 0.05, step = 0.001)]
    pinch_zoom_sensitivity: f32,
...
fn setup(
    ...
    // Touch-enabled orbit camera.
    // Touch is explicitly enabled and pinch sensitivity is tuned for tablets.
    // zoom_to_cursor gives a more natural feel on trackpads.
    ...
    let orbit = OrbitCamera::looking_at(DEFAULT_FOCUS, Vec3::new(0.0, 4.5, 9.0));

    commands.spawn((
        Name::new("Touch Orbit Camera"),
        orbit.clone(),
        settings.clone(),
    ...
    commands.insert_resource(OrbitPane {
        ...
        touch_enabled: settings.touch.enabled,
        pinch_zoom_sensitivity: settings.touch.pinch_zoom_sensitivity,
...
fn sync_pane_to_camera(
    ...
    for (mut orbit, mut settings) in &mut cameras {
        ...
        settings.touch.enabled = pane.touch_enabled;
        settings.touch.pinch_zoom_sensitivity = pane.pinch_zoom_sensitivity;
...
fn reflect_camera_to_pane(
    ...
    pane.touch_enabled = settings.touch.enabled;
